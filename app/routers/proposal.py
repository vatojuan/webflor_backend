...
def debug_dump_job(job_id: int, job: dict) -> None:
    logger.debug(f"Job {job_id} dump → "
...
def deliver(pid: int, sleep_first: bool) -> None:
    ...
    try:
        ...
        # 1) Estado actual
        cur.execute("SELECT status, job_id, applicant_id FROM proposals WHERE id=%s",
                    (pid,))
        row = cur.fetchone()
        if not row:
        ...
        if source == "admin":
            ...
            cur.execute('SELECT email, phone FROM "User" WHERE id=%s',
                        (owner_id,))
        ...
        cur.execute("UPDATE proposals SET status='sent', sent_at=NOW() WHERE id=%s",
                    (pid,))
        conn.commit()
...
def cancel(data: dict):
    ...
    try:
        ...
        cur.execute("UPDATE proposals SET status='cancelled', cancelled_at=NOW() WHERE id=%s",
                    (pid,))
        conn.commit()
