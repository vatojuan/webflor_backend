...
load_dotenv()
router = APIRouter(prefix="/api/admin/emails", tags=["email_db"])

# Auth / DB / Email settings
...
def extract_name(text: str) -> Optional[str]:
    prompt = [
        ...
        {"role": "user", "content": f"A partir del siguiente texto de CV, extrae sólo el nombre completo del candidato sin títulos ni cargos. Si no encuentras un nombre, responde 'No encontrado'.\n\n{text[:1000]}"}
    ]
    resp = openai_client.chat.completions.create(
        model="gpt-4-turbo", messages=prompt, max_tokens=10
    )
    name = resp.choices[0].message.content.strip()
    if not name or name.lower() == "no encontrado":
        return None
    return name
...
async def upload_files(files: List[UploadFile] = File(...)):
    ...
    for f in files:
        ...
        try:
            ...
            logs.append(f"Nombre: {name}")

            phone = extract_phone(text)
            logs.append(f"Teléfono: {phone}")
