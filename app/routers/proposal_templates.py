...
def set_default_template(template_id: int):
    ...
    try:
        ...
        cur.execute(
            "SELECT type FROM proposal_templates WHERE id = %s;",
            (template_id,)
        )
        row = cur.fetchone()
        if not row:
        ...
        # 2) Desmarcar todas las del mismo tipo y marcar sólo ésta
        cur.execute(
        ...
        conn.commit()
        return {
            "message": f"Plantilla {template_id} establecida como default para '{tpl_type}'"
        }
