...
from app.routers.proposal   import send_mail, send_whatsapp          # helpers ya probados
...
def _get_default_tpl(cur, tpl_type: str) -> Dict[str,str]:
    """Devuelve la plantilla predeterminada (puede ser admin-editable)."""
    cur.execute(
        """SELECT subject, body
...
def _job_contact_columns(cur) -> Dict[str,str]:
    cur.execute("""
        SELECT column_name
    ...
    email_c = "contact_email" if "contact_email" in cols else "contactEmail" if "contactEmail" in cols else None
    phone_c = "contact_phone" if "contact_phone" in cols else "contactPhone" if "contactPhone" in cols else None
    return {"email": email_c, "phone": phone_c}
...
def run_matching_for_job(job_id: int) -> None:
    ...
    éste acepta / se postula, así evitamos SPAM.
    """
    conn = cur = None
    try:
        ...
        # borramos matchings viejos
        cur.execute("DELETE FROM matches WHERE job_id = %s", (job_id,))

        # insertamos nuevos
        cur.execute("""
            INSERT INTO matches(job_id,user_id,score,status)
            SELECT %s, u.id, (1 - (u.embedding <=> %s)) AS score, 'pending'
              FROM "User" u
             WHERE u.embedding IS NOT NULL
        """, (job_id, job_emb))
        conn.commit()
        logger.info("run_matching_for_job: %d filas insertadas para job %d", cur.rowcount, job_id)
...
def resend_matching(mid: int):
    ...
    try:
        ...
        # datos matching + oferta + candidato
        cur.execute("""
            SELECT m.score,
                   j.id, j.title, j.userId,
                   j.contact_email, j.contactEmail,
        ...
        (score, job_id, job_title, owner_id,
         c_email1, c_email2, c_phone1, c_phone2,
         cand_name, cand_email, cv_url) = row
        ...
        # plantilla predeterminada (tipo automatic)
...
# ═══════════ Pre-cálculo (preview) ═══════════
@router.get("/job/{job_id}/match", summary="Preview de usuarios para un Job")
def match_for_job(job_id: int):
    ...
    try:
        ...
        job_emb=row[0]
        cur.execute("""
            SELECT u.id, u.email, (1 - (u.embedding <=> %s)) AS score
              FROM "User" u
             WHERE u.embedding IS NOT NULL
             ORDER BY score DESC
             LIMIT 100
...
@router.get("/user/{user_id}/match", summary="Preview de ofertas para un Usuario")
def match_for_user(user_id: int):
    ...
    try:
        ...
        user_emb=row[0]
        cur.execute("""
            SELECT j.id, j.title, (1 - (j.embedding <=> %s)) AS score
              FROM "Job" j
