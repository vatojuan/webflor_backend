...
def _insert_job(payload: Dict[str,Any],
                owner_id: int,
                source: str,
                label_default: str = "manual") -> Tuple[int,str]:
    """
    ...
    try:
        ...
        if has_is_paid: fields.append("is_paid"); values.append(is_paid)
        if email_col:
...
async def list_jobs(userId: Optional[int] = None):
    ...
    try:
        conn = get_db_connection(); cur = conn.cursor()
        has_source = job_has_column(cur,"source"); has_label=job_has_column(cur,"label")
        base = ['id','title','description','requirements','"expirationDate"','"userId"']
        sel  = base + [("source" if has_source else "NULL AS source"),
        ...
        params=()
        if userId is not None: sql+=' AND "userId"=%s'; params=(userId,)
        sql+=' ORDER BY id DESC'
        cur.execute(sql, params)
        cols=[d[0] for d in cur.description]
        return {"offers":[dict(zip(cols,r)) for r in cur.fetchall()]}
    finally:
        if cur: cur.close();  # pyright: ignore [reportPossiblyUnboundVariable]
        if conn: conn.close()
...
async def my_applications(current_user=Depends(get_current_user)):
    ...
    try:
        conn=get_db_connection(); cur=conn.cursor()
        cur.execute("""SELECT id, job_id AS "jobId", status,
...
async def create_admin_job(request: Request,
    ...
    job_id, _ = _insert_job(data, owner_id=owner_id, source=data.get("source","admin"),
