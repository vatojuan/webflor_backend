...
def get_current_admin(token: str = Depends(oauth2_scheme)) -> str:
    """Valida token de administrador y devuelve el 'sub' (e-mail)."""
    try:
...
# ════════════════════════════════════════
# GET /api/job/admin_offers   (requiere token)
# ════════════════════════════════════════
@router.get("/admin_offers", status_code=status.HTTP_200_OK)
def get_admin_offers(admin_sub: str = Depends(get_current_admin)):
    """
    Lista todas las ofertas (filtra expiradas según configuración).
    El token admin se pasa en Authorization: Bearer <token>.
    """
...
# ════════════════════════════════════════
# PUT /api/job/update-admin  (protección vía dependencias)
# ════════════════════════════════════════
...
async def update_admin_offer(request: Request):
    ...
    if source == "admin" and not contact_email:
        raise HTTPException(status.HTTP_400_BAD_REQUEST,
                            "Las ofertas del administrador requieren contactEmail")
    ...
    try:
        ...
        cur.execute(
            ...
            UPDATE public."Job"
               SET title=%s, description=%s, requirements=%s,
                   "expirationDate"=%s, "userId"=%s, source=%s, label=%s,
                   contact_email=%s, contact_phone=%s, embedding=%s
             WHERE id=%s
         RETURNING id,title,description,requirements,"expirationDate",
                   "userId",source,label,
                   contact_email AS "contactEmail",
                   contact_phone AS "contactPhone"
            """,
            (title, description, requirements, exp_date, user_id,
             source, label, contact_email, contact_phone, embedding, job_id)
        )
        upd = cur.fetchone()
        if not upd:
        ...
        keys = ["id","title","description","requirements","expirationDate",
        ...
        raise HTTPException(status.HTTP_500_INTERNAL_SERVER_ERROR,
                            f"Error interno al actualizar: {e}")
    finally:
        if cur:  cur.close()
        if conn: conn.close()

# ════════════════════════════════════════
# DELETE /api/job/delete-admin  (protección vía dependencias)
# ════════════════════════════════════════
